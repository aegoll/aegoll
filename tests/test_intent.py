...
from aegl import intent as intent_engine
from aegl.clock import FixedClock
from aegl.domain import Channel, Purpose, Vendor, Verdict
from aegl.intent import Intent, IntentStore
...
def request_for(aegl, resource="/market/snapshot", amount="0.001", **kw):
    return aegl.build_request(resource=resource, amount_usd=amount, vendor=SELLER, **kw)
...
def test_an_agent_with_no_intent_is_not_refused(aegl):
...
def test_an_agent_with_no_intent_says_so_explicitly(aegl):
    """`no check ran` and `check passed` must never look alike."""
    decision = aegl.decide(request_for(aegl))
    assert "no_intent_declared" in codes(decision)

    verdict = aegl.intent_for(request_for(aegl))
    assert verdict.governed is False
    assert verdict.as_dict()["intentId"] is None


def test_a_governed_action_is_marked_as_governed(aegl):
    aegl.intents.declare(agent_id="agent-1", purpose="buy data", maximum_usd="1.00", now=BASE)
    verdict = aegl.intent_for(request_for(aegl))
...
def test_an_expired_intent_authorises_nothing(aegl):
    ...
    expired = aegl.intents.all()[0]
    verdict = intent_engine.evaluate(request_for(aegl), expired, now=BASE)

    assert verdict.verdict is Verdict.REJECT
    assert "intent_expired" in {r.code for r in verdict.reasons}


def test_a_revoked_intent_authorises_nothing(aegl):
    declared = aegl.intents.declare(
        agent_id="agent-1", purpose="buy data", maximum_usd="1.00", now=BASE
    )
    assert aegl.intents.revoke(declared.intent_id) is True

    verdict = intent_engine.evaluate(
        request_for(aegl), aegl.intents.get(declared.intent_id), now=BASE
    )
    assert verdict.verdict is Verdict.REJECT
    assert "intent_revoked" in {r.code for r in verdict.reasons}


def test_revoking_keeps_the_record_of_it_having_been_granted(aegl):
    """Withdrawing authority is not the same as it never having existed."""
    declared = aegl.intents.declare(
        agent_id="agent-1", purpose="buy data", maximum_usd="1.00", now=BASE
    )
    aegl.intents.revoke(declared.intent_id)

    stored = aegl.intents.get(declared.intent_id)
    assert stored is not None
    assert stored.status == "revoked"
    assert stored.purpose == "buy data"


def test_an_expired_intent_is_not_returned_as_active(aegl):
    aegl.intents.declare(
        agent_id="agent-1", purpose="stale", maximum_usd="1.00",
        expires_at=BASE - timedelta(minutes=1), now=BASE - timedelta(days=1),
    )
    assert aegl.intents.active_for("agent-1", BASE) is None


# --- the action must be the kind of thing the intent is about -------------


def test_a_resource_outside_the_intent_is_reviewed_not_approved(aegl):
    aegl.intents.declare(
        agent_id="agent-1", purpose="buy market data", maximum_usd="1.00",
        allowed_resources=["/market/*"], now=BASE,
    )
    decision = aegl.decide(request_for(aegl, resource="/compute/batch"))

    assert decision.verdict is Verdict.REVIEW
    assert "intent_resource_mismatch" in codes(decision)


def test_a_resource_inside_the_intent_passes(aegl):
    aegl.intents.declare(
        agent_id="agent-1", purpose="buy market data", maximum_usd="1.00",
        allowed_resources=["/market/*"], now=BASE,
    )
    assert aegl.decide(request_for(aegl, resource="/market/signal")).verdict is Verdict.APPROVE


def test_an_empty_resource_list_means_unrestricted(aegl):
    """A real choice, and visible as one rather than an accident."""
    aegl.intents.declare(agent_id="agent-1", purpose="anything", maximum_usd="1.00", now=BASE)
    assert aegl.decide(request_for(aegl, resource="/anything/at/all")).verdict is Verdict.APPROVE


def test_a_category_outside_the_intent_is_reviewed(aegl):
    aegl.intents.declare(
    ...
    verdict = intent_engine.evaluate(
        request, aegl.intents.active_for("agent-1", BASE), now=BASE, category="gambling"
    )
    assert verdict.verdict is Verdict.REVIEW
    assert "intent_category_mismatch" in {r.code for r in verdict.reasons}


def test_an_intent_can_be_scoped_to_one_channel(aegl):
    ...
    verdict = intent_engine.evaluate(
        request, aegl.intents.active_for("agent-1", BASE), now=BASE
    )
...
def test_the_total_is_the_budget_not_the_per_action_figure(aegl):
    ...
    assert aegl.store.spent_under_intent(declared.intent_id) == 9_000

    decision = aegl.decide(request_for(aegl, amount="0.005"))
    assert decision.verdict is Verdict.REJECT
    assert "intent_budget_exceeded" in codes(decision)


def test_unsettled_spend_does_not_consume_the_intent(aegl):
    ...
    assert aegl.store.spent_under_intent(declared.intent_id) == 0


def test_a_per_action_ceiling_is_enforced_when_declared(aegl):
    aegl.intents.declare(
        agent_id="agent-1", purpose="buy data", maximum_usd="1.00",
...
def test_a_different_asset_is_refused_not_converted(aegl):
    ...
    verdict = intent_engine.evaluate(
        internal, aegl.intents.active_for("agent-1", BASE), now=BASE
    )
    assert verdict.verdict is Verdict.REJECT
...
def test_intent_can_only_narrow_never_widen(aegl):
    ...
    decision = aegl.decide(request_for(aegl, amount="5000.00"))
    assert decision.verdict is not Verdict.APPROVE


def test_a_clamp_by_intent_is_attributed(aegl):
...
def test_a_declared_intent_validates_against_the_schema(aegl):
    declared = aegl.intents.declare(
...
def test_the_schema_rejects_a_float_amount(aegl):
    declared = aegl.intents.declare(
...
def test_an_intent_round_trips_through_its_own_schema(aegl):
    declared = aegl.intents.declare(
