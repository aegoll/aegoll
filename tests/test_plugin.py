...
direction, that `aegl` never imports an agent.
"""
...
def test_a_ceiling_stop_is_journalled(gov):
    ...
    stops = [
        e for e in gov.aegl.audit.entries()
...
def test_report_is_plain_data(gov):
    """`aegl.ui.render()` takes this; a UI must not need AEGL's types."""
    import json
...
def _no_advisor_available(monkeypatch):
    ...
    Patching `aegl.advisors.keys.resolve_key` looks equivalent and is not: each
    ...
    """
    monkeypatch.setattr("aegl.plugin.build_advisor", lambda *a, **k: Unavailable())
...
def test_the_plugin_imports_no_framework_and_no_llm_sdk():
    ...
    reached only through `aegl.advisors`, lazily.
    """
    import ast
...
def test_precheck_previews_a_refusal_without_journalling_it(gov):
    ...
    """
    before = len(gov.aegl.audit.entries())
    ...
    assert len(gov.aegl.audit.entries()) == before, "the preview was journalled"
...
def test_an_override_is_journalled_with_its_reason(gov):
    ...
    overrides = [
        e.payload["settlement_update"] for e in gov.aegl.audit.entries()
