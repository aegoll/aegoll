...
- [x] A11.3a **The baseline is bounded by age as well as volume.** `agent_amounts` was `ORDER BY at DESC LIMIT 200` and nothing else — a *count* window, so history was evicted by volume rather than age. Measured: an agent with 40 varied purchases, given 200 trivial ones, kept 200 rows all of $0.001 and its real record was gone. Now bounded by 30 days first (matching every other aggregate), with the row cap kept as a memory guard and raised to 1,000. The 240-row case retains all 240 and the z-score measures again.

  **Truncation is reported rather than silent.** Hitting the cap sets `baseline_truncated`, which `risk` surfaces as a flag and in the term detail. A statistic over the most recent N actions is not the statistic it appears to be, and returning a volume-limited sample without saying so is how the original defect stayed hidden.
- [x] A11.3b **The fabricated `6.0` is gone, and no verdict moved.** With zero dispersion `amount_zscore` returned a hardcoded `6.0`, so $0.01 and $100,000 scored identically — a standard-deviation count that no standard deviation produced, flowing into `risk.score`, the journal and every report as though measured.

  It returns `None` now, and `dispersion_state()` names which of three reasons applies — `no_baseline`, `no_spread`, `measured`. **Absent is not unknown is not zero, applied to a statistic**: "nothing is known about this agent" and "every amount ever seen was identical" are opposite pieces of information and were the same return value.

  The signal the magnitude was carrying moved to `differs_from_flat_baseline()`, and `risk` scores that case at **exactly 1.0** — the same contribution `min(1.0, 6.0 / 4.0)` gave. **Collapsing it into the `no baseline` branch instead would have dropped the term to 0.0 and loosened the verdict**, which is the wrong direction for a governance layer and is now its own test. Four properties, each break-verified by reintroducing the bug.
...
- [~] A11.6 **Design doc written** — [`docs/design/evidence-anchoring.md`](docs/design/evidence-anchoring.md). Nothing implemented; the disclosure EVID-6 requires is already in place in three locations.

  Reading [EVID-6](https://github.com/aegoll/aegs/blob/main/spec/07-evidence.md) in full first settled more than expected: it requires the **disclosure, not the fix**, it already names the fix (an anchor committing to *length and head*, outside the agent's reach), and it already names the `head.json` trap. **tesoro is conformant as it stands.**

  What the doc establishes:
  - **The threat model is the design.** The adversary is *inside the host* and can rewrite anything the host can write. That disqualifies every anchor living inside it, and generalises A11.7 into a rule worth testing proposals against: **an anchor within the writer's authority is not an anchor.** A second table in the same database, a hash in a sibling log, a length in the journal's own header — the same mistake in different clothes
  - **The window is the honest limit.** Anything appended since the last publication is still silently truncatable, so an anchor does not make truncation detectable — it makes it detectable **beyond a bound the operator chose**. Identical shape to the count-envelope result. The adversary's best move is to truncate immediately, which is why publishing *on every refusal* is worth doing on its own: refusals are rare and are exactly the entries worth attesting
  - **Four outcomes, not two** — `consistent`, `truncated`, `diverged`, `unknown`. An unreachable sink must **not** read as valid. This project has shipped that exact collapse twice (`validate()` returning bare `False`; the cockpit shim reporting a missing UI module as an absent governance layer), so it is called out as the one defect that would make the feature worse than its absence
  - **Five candidate sinks with what each actually guarantees**, and a recommendation to ship **no bundled sink** — an append-only file is `head.json` at one remove in most deployments, with a config key making it look solved. The x402-rail option is flagged as attractive, nearly free, and structurally blind to refusals, which are the entries the adversary wants gone
- [ ] A11.6a **`EVID-6a` first, then vectors, then code.** Unlike the count envelope, this mechanism is genuinely unspecified: EVID-6 describes an anchor in prose but nothing fixes what it commits to, what the failure states are, or how verification reports them — so an anchor cannot be conformance-scored and a vector has no clause to cite. Recommended as a `MAY` modelled on `ENV-7`, whose fixed semantics are exactly why closing paced evasion needed no new clause
