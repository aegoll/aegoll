...
- [x] A11.6 **Anchored verification implemented** — `EVID-6a` in the standard, seven vectors, `engines/evidence/anchor.py`, `Governor.verify_anchored()`. Design doc at [`docs/design/evidence-anchoring.md`](docs/design/evidence-anchoring.md).

  Reading EVID-6 in full first was what shaped the work: it requires the **disclosure, not the fix**, already names the fix, and already names the `head.json` trap. So tesoro was conformant throughout, and this is capability beyond what AEGS requires.

  **Four outcomes, and `unknown` is never a pass.** An unreachable sink leaves the anchored claim unavailable — a third thing. Reporting it as consistent would mean anyone able to partition the verifier from the sink could also make a truncated journal verify. This codebase has shipped that collapse twice, so it is the property most heavily tested here.

  **It bounds rather than eliminates.** Anything appended since the anchor's last publication is unattested and still removable. A vector and a test both *assert* that limit — `anchor-truncation-inside-the-window-is-undetected` expects `consistent`, which is correct and is what "beyond a bound you chose" costs.

  **The head is recomputed from payloads, never read from the stored hashes.** An attacker who edits a payload leaves the stored hashes alone, so comparing against them would report an edited chain as matching its anchor.

  **No sink ships.** A11.7 generalised: an anchor within the writer's authority is not an anchor, and an append-only file beside the journal is rewritable by the agent's own user in most deployments. A test fails if a concrete sink ever appears in the package.

  Two things found while building it, both worth more than the feature:
  - **All seven vectors passed while the public API raised `TypeError`.** The runner builds plain dicts; `AuditLog.entries()` yields `AuditEntry` objects. A conformance suite exercising a different shape from the public API can be green over a broken feature — the same lesson as installing the wheel before believing the tests
  - **`Governor.decide()` and `verify()` were public and undocumented.** A new test now compares `docs/api-surface.md` §3 against the class in both directions, which is F-A12 turned into a check instead of a lesson
- [x] A11.7 **Enforced, not just written down.** `test_no_sink_ships_as_a_default` fails if any concrete class appears in the anchor module, and the rule it protects is stated as the general form: an anchor within the writer's authority is not an anchor
