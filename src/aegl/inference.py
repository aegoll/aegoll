...
class InferenceGate:
    ...
    def __init__(self, aegl: Aegl, vendor: Vendor = ANTHROPIC) -> None:
        self.aegl = aegl
        self.vendor = vendor
    ...
    def authorize_run(
        ...
        *,
        ...
        decision = self.aegl.authorize(request)
    ...
    def settle_run(
        ...
        self.aegl.store.mark_settled(request_id, None, success=success)
        # Correct the recorded amount from budget to actual.
        self.aegl.store._conn.execute(  # noqa: SLF001 - deliberate, same package
            "UPDATE transactions SET amount_atomic=? WHERE id=?",
            (actual_atomic, request_id),
        )
        self.aegl.store._conn.commit()  # noqa: SLF001
    ...
    def budget_state(self) -> dict[str, Any]:
        ...
        verdict = treasury_engine.evaluate(
            probe, snapshot, self.aegl.bundle.treasury_internal
        )
