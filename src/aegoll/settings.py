"""`aegoll.yaml` — the one file a user edits, and the only one they have to.
...
`aegoll.yaml` and `aegoll.json` are the same schema in two syntaxes (PLAN.md A0.5). One
loader, one parser, so the two cannot disagree — the extension only decides which name is
looked for first.

**`validate()` returns problems and never raises.** `aegoll check` needs every problem at
...
#: `aegoll init` writes and what the documentation shows.
CONFIG_NAMES = ("aegoll.yaml", "aegoll.yml", "aegoll.json")
...
class Config:
    ...
    def defaults(cls) -> "Config":
        ...
        A missing config is not an error. `pip install aegoll` then `Governor.load()`
    ...
    def policy(self) -> PolicyBundle:
        ...
        directory. Otherwise `aegoll check` and a running agent could read two different
