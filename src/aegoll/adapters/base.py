...
class RunGuard:
    ...
    **`should_stop()` exists because step ceilings are not spend ceilings.** Of the frameworks
    ...
    def stop_reason(self) -> str:
        """Which control stopped the run, or `""`. Not a boolean, because *why* is the useful
        part and a caller that only knows *whether* has to guess."""
        return self.stopped_by.attributed_control if self.stopped_by is not None else ""
