...
class Governor:
    ...
    def __init__(
        ...
        *,
        ...
        self.inference = InferenceGate(self.aegoll)
    ...
    def authorize_run(
        ...
        *,
        ...
        self.aegoll.audit.labels.setdefault("provider", provider)
    ...
    def register_identity(
        ...
        *,
        ...
        """
        return self.aegoll.identities.register(
            agent_id=agent_id or self.aegoll.agent_id,
            purpose=purpose,
            ...
            now=self.aegoll.clock.now(),
        )

    def identity(self, agent_id: str | None = None) -> Any | None:
        return self.aegoll.identities.get(agent_id or self.aegoll.agent_id)

    def set_identity_status(self, status: str, agent_id: str | None = None) -> bool:
        """Suspend, revoke or reactivate. Revocation is terminal."""
        return self.aegoll.identities.set_status(agent_id or self.aegoll.agent_id, status)
    ...
    def declare_intent(
        ...
        *,
        ...
        """
        return self.aegoll.intents.declare(
            agent_id=self.aegoll.agent_id,
            purpose=purpose,
    ...
    def active_intent(self) -> Any | None:
        return self.aegoll.intents.active_for(self.aegoll.agent_id, self.aegoll.clock.now())

    def revoke_intent(self, intent_id: str) -> bool:
        """Withdraw authority without deleting the record of it having been granted."""
        return self.aegoll.intents.revoke(intent_id)
    ...
    def precheck_run(
        ...
        decision = self.aegoll.decide(request)
    ...
    def wrap(self, client: Any, *, vendor: Vendor | None = None) -> Any:
        ...
        governed = GovernedBuyer(self.aegoll, client, vendor=vendor or self.seller)
    ...
    def report(self) -> dict[str, Any]:
        ...
        `aegoll.ui.render()` takes exactly this, which is what lets one panel serve
...
class _RecordingBuyer:
    ...
    async def get_paid(self, path: str) -> Any:
        before = len(self._gov.aegoll.audit.entries())
        try:
            ...
            raise
        for entry in self._gov.aegoll.audit.entries()[before:]:
