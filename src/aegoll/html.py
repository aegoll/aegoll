...
artifact you can attach to a ticket. `aegoll serve` (A10b) is the same renderer behind a second
...
def render(report: Report) -> str:
    ...
    Takes a `Report` rather than a layer, so `aegoll serve` can feed the same renderer live
...
<title>aegoll &mdash; {_e(data['policy']['name'])}</title>
<style>{_CSS}</style>
...
<header>
  ...
  <div class="sub">
    ...
    aegoll <code>{_e(versions['aegoll'])}</code> &middot;
...
<footer>
  Generated locally by <code>aegoll report --html</code>. No network access, no external
