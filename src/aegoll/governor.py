...
project". The code then grew a different shape underneath, so `from aegoll import Governor`
returned the internal rules evaluator and the README's own opening snippet raised
`AttributeError`. This module is the specified surface, implemented over `Aegoll`.
...
Why a facade at all, rather than telling people to use `Aegoll` directly:

* `Aegoll.authorize()` takes a `PaymentRequest` you must build first, with a `Vendor` object
  and a `Purpose` enum. Correct for a library, three imports and six lines for a first run.
* Configuration lives in `aegoll.yaml`, and `Aegoll(...)` does not read it. Every command in
...
The narrower door stays open: `Aegoll` remains public and unchanged, and anything this class
...
class Governor:
    ...
    **stable** — `docs/api-surface.md` §3. Construct with `load()`; `Governor()` takes a live
    `Aegoll` and exists for tests and for callers that already have one.
    ...
    def __init__(self, layer: "Aegoll", *, config: "Config | None" = None) -> None:
        self._layer = layer
        self._config = config
    ...
    def load(cls, path: str | Path | None = None) -> "Governor":
        """Read `aegoll.yaml` and build the layer it describes.

        The default constructor, because configuration belongs in a file rather than in a call
        with fourteen keyword arguments. A missing config is not an error: `pip install aegoll`
    ...
    def from_config(cls, config: "Config") -> "Governor":
        ...
        from .runtime import Aegoll, Paths  # noqa: PLC0415
        ...
        return cls(Aegoll(bundle=config.policy(), paths=paths), config=config)

    @staticmethod
    def _bare_layer() -> "Aegoll":
        from .runtime import Aegoll  # noqa: PLC0415
    ...
    def authorize(
        ...
        *,
        ...
        `Aegoll.authorize()` directly when you need to carry more about a counterparty than its
    ...
    def report(self, *, limit: int = 20) -> "Report":
        """Everything a reader needs, as plain data. Render with `aegoll.html.render()`."""
