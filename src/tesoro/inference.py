...
class InferenceGate:
    ...
    def __init__(self, tesoro: Tesoro, vendor: Vendor = ANTHROPIC) -> None:
        self.tesoro = tesoro
        self.vendor = vendor
    ...
    def authorize_run(
        ...
        *,
        ...
        decision = self.tesoro.authorize(request)
    ...
    def settle_run(
        ...
        self.tesoro.store.mark_settled(request_id, None, success=success)
        # Correct the recorded amount from budget to actual.
        self.tesoro.store._conn.execute(  # noqa: SLF001 - deliberate, same package
            "UPDATE transactions SET amount_atomic=? WHERE id=?",
            (actual_atomic, request_id),
        )
        self.tesoro.store._conn.commit()  # noqa: SLF001
    ...
    def budget_state(self) -> dict[str, Any]:
        ...
        verdict = treasury_engine.evaluate(
            probe, snapshot, self.tesoro.bundle.treasury_internal
        )
