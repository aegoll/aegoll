"""Headless entry point: `python -m tesoro.cli <command>`.
...
#: What `tesoro init` writes. Comments included on purpose: this is the first and often
#: only tesoro document a user reads closely, so the reasoning goes where they are already
#: looking rather than in documentation they have not opened.
_STARTER_CONFIG = '''\
# tesoro -- what to enforce, and where the rules live.
#
...
# into one -- `tesoro check` will tell you so.
...
def _aegl(args: argparse.Namespace, ephemeral: bool = False) -> Tesoro:
    ...
    2. **`tesoro.yaml`'s `policy:`** — the project's own pack. This is the normal case.
    ...
    edited `policies/default.yaml`, `tesoro check` confirmed the edit by name and content hash,
    and `tesoro decide` then governed the agent by the *packaged* numbers instead. Nothing
    ...
    if not args.policy:
        try:
            ...
            # No config, or a broken one. `tesoro check` is the command that reports why; the
            # others fall back to the packaged pack rather than refusing to start, which keeps
            # `tesoro decide` usable in a bare directory.
            config = None
    ...
    if ephemeral:
        ...
        # `logs/spend.jsonl`, see no error, and find their evidence in `./.tesoro` instead.
        paths = Paths.for_journal(config.evidence_path)
    else:
        paths = Paths.under()

    clock = FixedClock(BASE_TIME) if getattr(args, "fixed_time", False) else None
    return Tesoro(bundle=bundle, paths=paths, clock=clock)
...
def cmd_decide(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        decision = tesoro.decide(request) if args.dry_run else tesoro.authorize(request)
...
def cmd_audit(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"audit: {len(entries)} entries at {tesoro.paths.audit}")
...
def cmd_reviews(args: argparse.Namespace) -> int:
    ...
    try:
        if args.resolve:
            item = tesoro.queue.resolve(args.resolve, args.as_, by="cli", note=args.note)
            print("resolved:" if item else "not found:", args.resolve)
            return 0 if item else 1
        items = tesoro.queue.all() if args.all else tesoro.queue.pending()
        if args.json:
...
def cmd_bench(args: argparse.Namespace) -> int:
    ...
    tesoro = _aegl(args, ephemeral=True)
    try:
        ...
        for _ in range(args.n):
            ...
            latencies.append(tesoro.decide(req).latency_us)
...
def cmd_record(args: argparse.Namespace) -> int:
    ...
    if args.file:
        ...
        tesoro = _aegl(args)
        try:
            records = record_mod.records_from_journal(tesoro.audit.entries())
        finally:
            tesoro.close()
...
def cmd_intent(args: argparse.Namespace) -> int:
    ...
    try:
        if args.revoke:
            ok = tesoro.intents.revoke(args.revoke)
            print(f"  {'revoked' if ok else 'not found'}: {args.revoke}")
            return 0 if ok else 1

        if args.declare:
            expires = (
                tesoro.clock.now() + timedelta(hours=args.expires_in)
                if args.expires_in
                else None
            )
            intent = tesoro.intents.declare(
                agent_id=args.agent,
        ...
        if not intents:
            ...
            return 0
        for i in intents:
            spent = tesoro.store.spent_under_intent(i.intent_id)
            print(
...
def cmd_identity(args: argparse.Namespace) -> int:
    ...
    try:
        if args.status:
            ok = tesoro.identities.set_status(args.agent, args.status)
        ...
        if args.register:
            identity = tesoro.identities.register(
                ...
                policy_version=tesoro.bundle.hash,
                now=tesoro.clock.now(),
            )
        ...
        identities = tesoro.identities.all()
        if args.show:
            target = tesoro.identities.get(args.agent)
            if target is None:
...
def cmd_init(args: argparse.Namespace) -> int:
    """Scaffold `tesoro.yaml` and a starter policy pack in the working directory.
    ...
    config_path = target / "tesoro.yaml"
    ...
    print()
    print("next:  tesoro check     # validate before an agent holds a wallet")
    return 0
...
def cmd_report(args: argparse.Namespace) -> int:
    ...
    tesoro = _aegl(args)
    try:
        report = reporting.build(tesoro, profile=profile, limit=args.limit)
    finally:
        tesoro.close()
    ...
    if args.html:
        ...
        if args.output:
            ...
            # Defaults to stdout so it pipes. `tesoro report --html > spend.html` should
...
def cmd_conformance(args: argparse.Namespace) -> int:
    ...
    tesoro = _aegl(args)
    try:
        records = record_mod.records_from_journal(tesoro.audit)
    finally:
        tesoro.close()
    ...
    if not records:
        print()
        print("nothing to score yet. Run some decisions first -- try `tesoro decide`.")
        return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = _Parser(prog="tesoro", description="AEGL Phase 1 (deterministic)")
    ...
    ini = sub.add_parser("init", help="scaffold tesoro.yaml and a starter policy")
    ...
    chk.add_argument("--config", help="path to tesoro.yaml or tesoro.json")
    ...
    rep.add_argument("--config", help="path to tesoro.yaml or tesoro.json")
    ...
    pex.add_argument("--config", help="path to tesoro.yaml or tesoro.json")
...
def main() -> None:
    ...
    # `tesoro --json report` and `tesoro report --json` must behave identically. argparse
