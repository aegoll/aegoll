...
Everything above this line (`app.py`, `cli.py`, the x402 adapter) talks to `Tesoro`
...
#:
#: This was `Path.cwd() / ".tesoro"` evaluated at import time and captured in `under()`'s
...
class Paths:
    ...
    def for_journal(cls, journal: Path | str) -> "Paths":
        ...
        derives the directory, because `tesoro.yaml` says `evidence: journal: <file>` and a
