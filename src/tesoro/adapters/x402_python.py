...
def _ensure_core_importable() -> None:
    ...
    if importlib.util.find_spec("x402_core") is None:
        raise ImportError(
            ...
            "Install the rail extra (`pip install tesoro[x402]`), or add the shared "
    ...
    async def get_paid(self, path: str) -> Any:
        ...
        if quote is not None and not hasattr(quote, "price_usd"):
            ...
            raise TypeError(
                ...
                "quote that has no price. See tesoro.plugin.Quote."
            )
        amount_atomic = (
...
def build_governed_buyer(
    ...
    *,
    ...
    return GovernedBuyer(tesoro, buyer, vendor=vendor)
